...
=====================
*/
typedef void (*Callback_assertionReportFn)(const char *szMessage, const char *pszFilename, unsigned int iLine);
...
#define dpreleaseassert(x) (void)((!!(x)) || (AssertFailed(__FILE__, __LINE__, #x)));
...
#define compile_time_assert(x)                                                                                         \
    typedef compile_time_assert_test<sizeof(compile_time_assert_failed<(bool)(x)>)> compile_time_assert_join(          \
        compile_time_assert_typedef_, __LINE__)
#endif
