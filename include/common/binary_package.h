...
public:
   ...
   CBinaryPackage() { InitWrite(NULL, 0); }
   CBinaryPackage(ubyte* pData, size_t nLength) { InitWrite(pData, nLength); }
   CBinaryPackage(const ubyte* pData, size_t nLength) { InitRead(pData, nLength); }
   ...
   void           SetHasChanged(bool b) { bChanged = b; }
