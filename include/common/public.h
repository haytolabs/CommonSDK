...
template <typename _type_> bool IsSignedType(const _type_ t)
{
    return _type_(-1) < 0;
}
...
#define DECLARE_IMPORT(m_pszClassname, inetfacename, exportname)                                                       \
    ...
    static CComponentRegistration __##m_pszClassname##__AddImportableComponent(__##m_pszClassname##__CreateInstance,   \
                                                                               exportname);
